...
static void _TestTCPSocket() {
    ...
    //    MWL_INFO("s1 is bound to %s@%d", s1.LocalAddress().Host().C_Str(), s1.LocalAddress().Port());
    //}
    s1.Listen();
    MWL_INFO("s1 is listening at %s@%d", s1.LocalAddress().Host().C_Str(), s1.LocalAddress().Port());
    ...
    if (ERR_NONE == s2.Bind("localhost")) {
        MWL_INFO("s2 is bound to %s@%d", s2.LocalAddress().Host().C_Str(), s2.LocalAddress().Port());
    }
    ...
    if (ERR_NONE == s2.Connect(s1.LocalAddress())) {
        ...
        if (ERR_NONE == s1.Accept(s3)) {
            MWL_INFO("s2 at %s:%d is connected to %s:%d",
                s2.LocalAddress().Host().C_Str(), s2.LocalAddress().Port(),
                s2.PeerAddress().Host().C_Str(), s2.PeerAddress().Port());

            MWL_INFO("s3 at %s:%d is connected to %s:%d",
                s3.LocalAddress().Host().C_Str(), s3.LocalAddress().Port(),
                s3.PeerAddress().Host().C_Str(), s3.PeerAddress().Port());
            char buf[256] = {0};
...
static void _TestUDPSocket() {
    ...
    if (n > 0) {
        MWL_INFO("%d bytes has been sent to %s:%d", n, s1.LocalAddress().Host().C_Str(), s1.LocalAddress().Port());
        SockAddress srcAddr;
        s1.RecvFrom(buf, 256, srcAddr);
        MWL_INFO("s1 received %s from %s:%d", buf, srcAddr.Host().C_Str(), srcAddr.Port());
    }
}
...
static int32_t MySysServer(ThreadContext *pCtx) {
    ...
    svrSock.Listen();
    MWL_INFO("my daytime server is running at: %s:%d", svrSock.LocalAddress().Host().C_Str(), svrSock.LocalAddress().Port());
    TimeSpan timeout(10);
