...
#define _LEGAL_ALT(x)       { if (alt_format & ~(x)) return (0); }
    ...
    static unsigned char *_strptime(const unsigned char *buf, const char *fmt, struct tm *tm, struct century_relyear *cr) {
         ...
         */
        if (cr->relyear != -1) {
            if (cr->century == __TM_YEAR_BASE) {
                if (cr->relyear <= 68) {
                    tm->tm_year = cr->relyear + 2000 - __TM_YEAR_BASE;
                } else {
                    tm->tm_year = cr->relyear + 1900 - __TM_YEAR_BASE;
                }
            } else {
                tm->tm_year = cr->relyear + cr->century - __TM_YEAR_BASE;
            }
        }
