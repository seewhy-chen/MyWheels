...
namespace mwl {
    ...
    int32_t FileSystem::DuplicateFile(const String &srcPath, const String &dstPath, FileOperatePolicy policy) {
        ...
        if (IsDir(dstPath)) {
            String fileName, ext;
            ParsePath(srcPath, nullptr, &fileName, &ext);
            dstFilePath = MakePath(dstPath, fileName, ext);
        }
    ...
    int32_t FileSystem::ParsePath(const String &path, String *pDirName, String *pFileName, String *pExt) {
        ...
        if (splitted.Size() > 1) {
            splitted = splitted[1].RSplit(_ExtSeperator(), 1);
            if (pFileName) {
    ...
    String FileSystem::MakePath(const String &dirName, const String &fileName, const String &ext) {
        ...
        if (!fullPath.EndsWith(_PathSeperator())) {
            fullPath += _PathSeperator();
        }
        fullPath += fileName;
        if (!ext.Empty()) {
